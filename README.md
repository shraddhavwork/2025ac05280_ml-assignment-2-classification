...
| Random Forest (Ensemble) | 0.8995 | 0.9296 | 0.5596 | 0.6616 | 0.6063 | 0.5517 |
...
| Random Forest (Ensemble) | Combines many decision trees and uses class weighting for the imbalanced target. It obtains Accuracy=0.8995, AUC=0.9296, F1=0.6063 and MCC=0.5517, providing a more stable non-linear model than one tree. |
| Overall Winner | Random Forest (Ensemble) has the highest F1 score (0.6063) on the fixed test split. Highest AUC: Random Forest (Ensemble); highest Recall: Random Forest (Ensemble); highest MCC: Random Forest (Ensemble). Because subscription is the minority outcome, F1 and MCC are considered together with Accuracy and AUC rather than choosing a model from Accuracy alone. |
